...
static void guided_takeoff_start(float final_alt_above_home)
{
    ...
    wp_nav.set_wp_destination(target_pos);
		gcs_send_text_P(SEVERITY_HIGH,PSTR("Finished initializing wpnav destination in control_guided.pde"));

    // initialise yaw
    set_auto_yaw_mode(AUTO_YAW_HOLD);
		gcs_send_text_P(SEVERITY_HIGH,PSTR("Finished initializing yaw mode in control_guided.pde"));

    // tell motors to do a slow start
    motors.slow_start(true);
		gcs_send_text_P(SEVERITY_HIGH,PSTR("Finished doing slow start in control_guided.pde"));
}
