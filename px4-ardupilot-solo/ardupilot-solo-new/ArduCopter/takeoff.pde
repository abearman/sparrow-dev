...
static bool do_user_takeoff(float takeoff_alt_cm, bool must_navigate)
{
        ...
        switch(control_mode) {
            case GUIDED:
								gcs_send_text_P(SEVERITY_HIGH,PSTR("Got to GUIDED mode in takeoff.pde"));
                set_auto_armed(true);
								gcs_send_text_P(SEVERITY_HIGH,PSTR("Set auto armed in takeoff.pde"));
                guided_takeoff_start(takeoff_alt_cm);
								gcs_send_text_P(SEVERITY_HIGH,PSTR("Passed guided_takeoff_start in takeoff.pde"));
                return true;
            case LOITER:
            case POSHOLD:
            case ALT_HOLD:
